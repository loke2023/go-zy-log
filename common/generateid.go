...
func NewWorker(workerId int64) (*GenerateWorker, error) {
...
func (w *GenerateWorker) GetId() int64 {
	// 获取id最关键的一点 加锁 加锁 加锁
	w.mu.Lock()
	...
	if w.timestamp == now {
		...
		w.number = 0
		// 下面这段代码看到很多前辈都写在if外面，无论节点上次生成id的时间戳与当前时间是否相同 都重新赋值
		// 这样会增加一丢丢的额外开销 所以我这里是选择放在else里面
